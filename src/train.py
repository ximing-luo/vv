...
import glob
import argparse
# 将项目根目录（AGI）和 src 目录添加到 sys.path
...
class ModelTrainer:
    ...
    def prepare_data(self):
        ...
        if len(self.val_dataset) > MAX_EVAL_SAMPLES:
            self.val_dataset, _ = torch.utils.data.random_split(
                self.val_dataset, 
                [MAX_EVAL_SAMPLES, len(self.val_dataset) - MAX_EVAL_SAMPLES]
            )
            print(f"[Data] 验证集过大，已截断至 {MAX_EVAL_SAMPLES} 条样本用于快速评估")
    ...
    def train(self):
        ...
        # 计算动态 batch size，确保为整数
        # 这里的 4096 是目标单卡处理 token 数，64 是目标总有效 Batch Size (序列数)
        per_device_train_batch_size = max(1, int(4096 // (self.model.config.max_seq_len*self.model.config.rope_ntk_alpha)))
        gradient_accumulation_steps = max(1, int(64 // per_device_train_batch_size))
        # per_device_train_batch_size = 64
        # gradient_accumulation_steps = 1
        print(f"[System] 动态计算得到的 Batch Size: {per_device_train_batch_size}")
        print(f"[System] 动态计算得到的 Gradient Accumulation Steps: {gradient_accumulation_steps}")
        # 4. 训练参数设置
        training_args = TrainingArguments(
            ...
            per_device_train_batch_size=per_device_train_batch_size, # 单卡训练 Batch Size
            gradient_accumulation_steps=gradient_accumulation_steps, # 梯度累积步数，变相扩大 Batch Size
            weight_decay=0.01, # 权重衰减 (L2 正则化)
            warmup_steps=1000, # 预热步数 (Warmup)
            lr_scheduler_type="cosine", # 学习率调度策略 (余弦退火)
            # 3. 评估配置 (Evaluation)
            per_device_eval_batch_size=per_device_train_batch_size, # 单卡评估 Batch Size
    ...
    def _get_latest_checkpoint(path):
        ...
        return max(valid_checkpoints, key=lambda x: int(x.split('-')[-1])) if valid_checkpoints else None
    ...
    def _load_model_weights(model, path):
        ...
        try:
            if os.path.exists(st_path):
                model.load_state_dict(load_file(st_path))
                print(f"[System] 成功加载 safetensors 权重: {st_path}")
                return True
...
def train(mode, num_train_epochs=1):
    """
    保持向后兼容的 train 函数入口
    """
...
if __name__ == "__main__":
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="AGI 模型训练脚本")
    parser.add_argument(
        "--mode", 
        type=str, 
        default="finetune", 
